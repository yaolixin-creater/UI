  ...
  <div>
    ...
    <div style="margin-left: 20px; margin-top: 20px; font-size: 14px">
      <el-form
        ...
        size="mini"
      >
        <el-form-item label="分组名称" :required="false" prop="groupId">
      ...
      <template>
        <el-dialog
              ...
              width="30%"
              center>
              <el-input v-model="addGroupFrom.addGroupName" placeholder="请输入分组名称"></el-input>
              <el-input v-model="addGroupFrom.addGroupDesc" placeholder="描述"></el-input>
      ...
      <template>
        <el-dialog
              :title="'【'+groupConfig+'】' + '的分组信息修改'"
              :visible.sync="centerDialogVisible2"
              width="30%"
              center>
              <el-input v-model="modGroupFrom.modGroupName" placeholder="请输入分组名称"></el-input>
              <el-input v-model="modGroupFrom.modGroupDesc" placeholder="描述"></el-input>
              <span slot="footer" class="dialog-footer">
...
import {
  backendManageDelgroup,
  backendManageQuerygroups,
  backendManagecreategroup,
  backendManagemodgroup
} from '@/api/getData'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    async handleDelete (index, row) {
      try {
        const resp = await backendManageDelgroup({
    ...
    async modgroup () {
      ...
      try {
        ...
        const resp = await backendManagemodgroup(data)
        if (resp.code === 0) {
          this.$message({
            showClose: true,
            type: 'success',
    ...
    async addgroup () {
      ...
      try {
        ...
        const resp = await backendManagecreategroup(data)
        if (resp.code === 0) {
          this.$message({
            showClose: true,
            type: 'success',
    ...
    async onQuerygroup () {
      ...
      try {
        ...
        const resp = await backendManageQuerygroups(data2)
        if (resp.code === 0) {
          resp.data.forEach((item, index) => {
            const data = {}
            const operation = {}
            data.processBar = '已完成'
            operation.groupId = data.groupId = item.group_id
            operation.groupName = data.groupName = item.group_name
          ...
          this.$message({
            ...
            type: 'success',
            message:
              '查询成功, 共计 ' + this.groupTableData.length + ' 条分组...'
          })
        } else {
          this.$message.error(resp.ErrMsg)
        }
